...
import { runSessionPrompt } from './telegram-session.js';
...
async function answerWithClaude(chatId, prompt) {
  ...
  try {
    ...
    const r = await runSessionPrompt(prompt, {
      sessionId: cfg.lastSessionId,
      run: runClaude,
      saveSession(id) { cfg.lastSessionId = id; saveConfig(); },
    });
    if (r.ok) {
      await sendText(chatId, r.text);
    } else {
      await sendText(chatId, `⚠️ Claude-Fehler: ${r.err}\nDer Auftrag wurde nicht automatisch wiederholt; der Verlauf bleibt erhalten. Falls die Sitzung nicht mehr existiert, mit /new neu beginnen.`);
    }
  } finally {
